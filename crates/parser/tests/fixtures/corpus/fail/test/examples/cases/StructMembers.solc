...
instance Unit:MemorySize { function memorySize(x : Proxy(Unit)) -> Word { return 0; } }
instance Uint256:MemorySize { function memorySize(x : Proxy(Uint256)) -> Word { return 32; } }
instance Bool:MemorySize { function memorySize(x : Proxy(Bool)) -> Word { return 32; } }
instance Bytes32:MemorySize { function memorySize(x : Proxy(Bytes32)) -> Word { return 32; } }

/// Memory size of pairs
instance Pair(a,b):MemorySize {
    function memorySize(x : Proxy((a,b))) -> Word
    {
        let pa:Proxy(a);
        let pb:Proxy(b);
...
{
    function load(x : MemberAccess(Memory(structType), fieldType, Memory(ty))) -> ty {
        let ptr:Word;
