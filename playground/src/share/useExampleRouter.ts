  ...
  useEffect(() => {
    ...
    const applySelection = (view: ExampleView): void => {
      ...
      useWorkspaceStore.setState({
        selectedTestId: test?.id ?? null,
        ...(test ? { runActivity: "tests" as const } : {}),
        callDraft: test?.invocation
          ? { ...state.callDraft, ...test.invocation, contract: test.contract, constructorArguments: "[]" }
          : { ...state.callDraft, contract: contract?.name ?? "", signature: method?.signature ?? "",
    ...
    const followRoute = (): void => {
      ...
      try {
        ...
        useWorkspaceStore.setState({ outputTab: tabs.includes(tab as OutputTab) ? tab as OutputTab : "execution",
          runActivity: view.view === "tests" ? "tests" : "calls", selectedTestId: null });
